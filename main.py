"""
This program, "FRED", uses the prebuilt AI model from google titled TensorFlow
Using the Fred neural network will take price, VIX, volality, date, as inputs,
then output a predicted price for a stock. This will then pass into a new function 
where I will calcuate the expected movement of the Dow Jones after 30 days.

This program is hard to run, as it requires a lot of computing power

Free to edit code as seen, I have some function I declared that I don't use becuase of
computing power, but if you know how to edit it, you can.
"""
...
def load_data(ticker, n_steps=50, scale=True, shuffle=True, lookup_step=1, split_by_date=True,
              test_size=0.2, feature_columns=['adjclose', 'volume', 'open', 'high', 'low', 'VIX']):
    ...
    if isinstance(ticker, str):
        ...
        vo = pd.DataFrame(vo)
        vo = vo.drop(columns=["open", 'high', "low",
                     "volume", "ticker", "close"])

        # Gets VIX index and adds it
        # Above 20 VIX means high volatilty, lower means less risk
        vo = vo.rename(columns={'adjclose': 'VIX'})
...
def DOW_test1():
    AXP = (Individual_Stocks("AXP")[1])
    AMGN = (Individual_Stocks("AMGN")[1])
    AAPL = (Individual_Stocks("AAPL")[1])
    BA = (Individual_Stocks("BA")[1])
    CAT = (Individual_Stocks("CAT")[1])
    CSCO = (Individual_Stocks("CSCO")[1])
    CVX = (Individual_Stocks("CVX")[1])
    GS = (Individual_Stocks("GS")[1])
    HD = (Individual_Stocks("HD")[1])
    HON = (Individual_Stocks("HON")[1])
    IBM = (Individual_Stocks("IBM")[1])
    INTC = (Individual_Stocks("INTC")[1])
    JNJ = (Individual_Stocks('JNJ')[1])
    KO = (Individual_Stocks('KO')[1])
    JPM = (Individual_Stocks('JPM')[1])
    MCD = (Individual_Stocks('MCD')[1])
    MMM = (Individual_Stocks('MMM')[1])
    MRK = (Individual_Stocks('MRK')[1])
    MSFT = (Individual_Stocks('MSFT')[1])
    NKE = (Individual_Stocks('NKE')[1])
    PG = (Individual_Stocks('PG')[1])
    TRV = (Individual_Stocks('TRV')[1])
    UNH = (Individual_Stocks('UNH')[1])
    CRM = (Individual_Stocks('CRM')[1])
    VZ = (Individual_Stocks('VZ')[1])
    V = (Individual_Stocks('V')[1])
    WBA = (Individual_Stocks('WBA')[1])
    WMT = (Individual_Stocks('WMT')[1])
    DIS = (Individual_Stocks('DIS')[1])
    DOW = (Individual_Stocks('DOW')[1])
